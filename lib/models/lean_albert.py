...
from transformers.models.albert.modeling_albert import (
    AlbertMLMHead, AlbertSOPHead, ALBERT_START_DOCSTRING, ACT2FN, AlbertLayerGroup,
    AlbertTransformer, AlbertForPreTraining, AlbertModel)
...
def get_attention_core(config: LeanAlbertConfig):
    if config.position_embedding_type == 'absolute':
        ...
        return RotaryAttentionCore(config.hidden_size, config.num_attention_heads, rotary_emb,
...
class LeanAlbertLayer(nn.Module):
    def __init__(self, config: LeanAlbertConfig):
        ...
        self.attention = LeanSelfAttention(config.hidden_size, config.num_attention_heads,
                                           attention_core=get_attention_core(config),
                                           hidden_dropout_prob=config.hidden_dropout_prob,
                                           layer_norm_eps=config.layer_norm_eps)

        self.ffn = LeanFFN(config.hidden_size, config.intermediate_size,
                           activation=ACT2FN[config.hidden_act], gated=config.hidden_act_gated,
